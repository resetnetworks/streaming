...
const SongUploadStep = ({
  ...
  useEffect(() => {
    if (
      ...
      currentUploadIndex < songsWithStatus.length
    ) {
  ...
  useEffect(() => {
    if (uploadedSongs.length > 0) {
      setSongsWithStatus((prev) =>
        prev.map((song, index) => {
          const uploadedSong = uploadedSongs.find(
            (us) => us.originalIndex === index,
          );
          if (uploadedSong) {
            return {
              ...song,
  ...
  const saveEditSong = (id) => {
    ...
    setSelectedSongs((prev) =>
      prev.map((song) =>
        song.id === id ? { ...song, name: editSongName.trim() } : song,
      ),
    );
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-gray-400 mb-6 max-w-md mx-auto">
          Upload multiple audio files to add to your album. You can select WAV,
          AIFF or files FLAC.
        </p>
        ...
        <p className="text-gray-500 text-sm mt-4">
          Selected: {selectedSongs.length} file
        ...
        <div className="bg-gray-900/50 rounded-lg border border-gray-700 p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-gray-400 text-sm">
              {songsWithStatus.filter((s) => s.status === "success").length} of{" "}
              {songsWithStatus.length} uploaded
            </span>
          </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      {/* Song number */}
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${statusInfo.color} bg-black/30`}
                      >
                        {index + 1}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {isEditing ? (
                          ...
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {statusInfo.icon && (
                              <div
                                className={`p-1 rounded ${statusInfo.bgColor}`}
                              >
                                {statusInfo.icon}
                              </div>
                            )}
  ...
  Editable
          ...
          <div className="mt-4 flex justify-between items-center">
            <div className="text-gray-400 text-sm">
              {songsWithStatus.length} song
              {songsWithStatus.length !== 1 ? "s" : ""} selected
              {editingSongId && (
            ...
            <div className="flex gap-3">
              ...
              {isUploading && (
                <div className="px-6 py-2 bg-blue-700 text-blue-300 rounded-full flex items-center gap-2">
                  ...
                  <span>
                    Uploading {currentUploadIndex + 1} of{" "}
                    {songsWithStatus.length}
                  </span>
                </div>
              )}
            </div>
